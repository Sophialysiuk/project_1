...
back = (100, 255, 255) #колір фону (background)
...
score_text = Label(270,0,50,50,back)
score_text.set_text('Рахунок:',40, DARK_BLUE)
score_text.draw(20,20)
 
score = Label(430,20,50,40,back)
...
while True:
    '''Відмальовка карток та відображення кліків'''
    if wait == 0:
    ...
    if int (new_time) - int(cur_time) == 1: #перевіряємо чи є різниця в одну секунду між старим і новим часом
    ...
    if points >= 5:
        ...
        win.draw(140,180)
        result_time = Label(90,230,250, LIGHT_GREEN)
        result_time.set_text("Час проходження: " + str(int(new_time - start_time)) + "секунд", 40, DARK_BLUE)
